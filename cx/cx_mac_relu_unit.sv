module cx_mac_relu_unit 
  ...
  logic [C_M_CXU_DATA_W-1:0] n_data;

  assign n_data = acc + (s_cxu.req_data0 * s_cxu.req_data1);

  always_ff @ (posedge i_clk) begin
    if (s_cxu.req_valid & s_cxu.req_ready) begin
      s_cxu.resp_id     <= s_cxu.req_id;
      s_cxu.resp_status <= 0;
      if (n_data[C_M_CXU_DATA_W-1]) begin
        s_cxu.resp_data <= '0;
      end else begin
        s_cxu.resp_data <= n_data;
      end
    end
  end
